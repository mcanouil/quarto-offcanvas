...
- fix: Read the options through the schema, so `scroll: yes` and `keyboard: no` take effect. Comparing the document text accepted `true` and `false` only. (#43)
...
- build: Update the vendored Lua modules to 2.5.0, which adds the accessor that reads what the schema resolves an option to. The schema validator moves to 3.6.3, whose vendored file is unchanged. The extension no longer vendors `metadata.lua`. (#43)
