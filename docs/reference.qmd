...
With `overtake-margins`, every `.column-margin` and `.aside` element becomes an offcanvas panel with its trigger in the margin.
...
| `responsive` breakpoints | Bootstrap 5.2 |
| `animation` presets | Bootstrap 5.3, which provides `--bs-offcanvas-transition-duration` |
