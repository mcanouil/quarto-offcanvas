...
The heading level does not reach the output.
The extension reads the text of the first heading and removes it from the body, so `#` and `##` give the same panel.
...
- A panel takes its title from a heading or the `title` attribute; without either the header is empty.
  The panel still points at the missing title with `aria-labelledby`, so it has no accessible name.
